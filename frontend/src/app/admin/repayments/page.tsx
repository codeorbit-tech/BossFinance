...
function RecordPaymentModal({ loan, onClose, onSuccess }: {
  loan: Repayment; onClose: () => void; onSuccess: (closure?: ClosureResponse) => void;
}) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const res = await repaymentsApi.record({ loanId: loan.id, amount: parseFloat(amount), method, reference });
      if (res.data.closure?.required) {
        toast.success(res.data.repayment ? 'Last EMI recorded. Settle penalty to complete the loan.' : 'Settle penalty to complete this loan.');
      } else {
        toast.success(res.data.repayment ? `Payment of ₹${parseFloat(amount).toLocaleString('en-IN')} recorded!` : (res.data.message || 'Loan is already completed.'));
      }
      onSuccess(res.data.closure);
      onClose();
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-300">
        <div className="p-6 bg-gradient-to-r from-accent to-on-primary-container text-white rounded-t-2xl">
          ...
          <p className="text-sm text-white/80 mt-0.5">{loan.customerName} — {loan.customerId}</p>
        </div>
...
function PenaltyClosureModal({ loan, totalPenaltyDue, onClose, onSuccess }: {
  loan: Repayment; totalPenaltyDue: number; onClose: () => void; onSuccess: () => void;
}) {
  const [discount, setDiscount] = useState('0');
  const [method, setMethod] = useState('CASH');
  const [loading, setLoading] = useState(false);
  const rawDiscount = parseFloat(discount || '0') || 0;
  const discountValue = Math.max(0, rawDiscount);
  const payable = Math.max(0, totalPenaltyDue - discountValue);

  const handleSubmit = async () => {
    if (discountValue > totalPenaltyDue) {
      toast.error('Discount cannot be more than total penalty');
      return;
    }

    setLoading(true);
    try {
      await repaymentsApi.settlePenalties({
        loanId: loan.id,
        amount: payable,
        discount: discountValue,
        method,
        description: `Final penalty settlement for ${loan.customerName}`,
      });
      toast.success('Penalty settled and loan completed.');
      onSuccess();
      onClose();
    } catch (err: any) {
      toast.error(err.response?.data?.error || 'Failed to settle penalty');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-300">
        <div className="p-6 bg-tertiary text-white rounded-t-2xl">
          <h3 className="font-bold text-lg">Loan Closure Penalty</h3>
          <p className="text-sm text-white/80 mt-0.5">{loan.customerName} - {loan.customerId}</p>
        </div>
        ...
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-3 gap-3 text-center">
            <div className="bg-slate-50 rounded-xl p-3">
              ...
              <p className="text-sm font-black text-error">₹{totalPenaltyDue.toLocaleString('en-IN')}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3">
              <p className="text-[10px] font-bold text-slate-500 uppercase">Discount</p>
              <p className="text-sm font-black text-amber-600">₹{discountValue.toLocaleString('en-IN')}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3">
              <p className="text-[10px] font-bold text-slate-500 uppercase">Payable</p>
              <p className="text-sm font-black text-accent">₹{payable.toLocaleString('en-IN')}</p>
            </div>
          </div>

          <div>
            <label className="text-[10px] font-bold text-slate-500 uppercase block mb-1">Discount Amount</label>
            <input
              type="number"
              min="0"
              max={totalPenaltyDue}
          ...
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs font-bold text-amber-800">
            Loan will be completed only after this penalty is paid or fully discounted.
          </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              ...
              className="flex-1 py-3 rounded-xl bg-accent text-white text-sm font-bold hover:opacity-90 transition-all disabled:opacity-50"
            >
              {loading ? 'Settling...' : payable === 0 ? 'Waive & Complete' : 'Collect & Complete'}
            </button>
          </div>
        </div>
      </div>
...
function CustomerSearch({ onSelect }: { onSelect: (loan: Repayment) => void }) {
  ...
  useEffect(() => {
    const search = async () => {
      ...
      setLoading(true);
      try {
        const res = await customersApi.list({ search: query, limit: '5' });
  ...
  const handleSelect = async (customer: SearchResult) => {
    try {
      const res = await customersApi.get(customer.id);
      const activeLoan = res.data.customer.loans.find((l: any) => l.status === 'ACTIVE' || l.status === 'OVERDUE' || l.status === 'CLOSURE_PENDING');
      if (!activeLoan) {
      ...
      onSelect({
        ...
        totalPaid: paid,
        outstanding: activeLoan.amount - paid,
        emi: activeLoan.emi,
        lastPayment: activeLoan.repayments[0]?.paidAt || null,
        nextDueDate: activeLoan.nextDueDate,
        status: activeLoan.status === 'CLOSURE_PENDING' ? 'PENALTY_PENDING' : activeLoan.status,
      ...
      toast.error('Failed to load customer loan details');
    }
  };
  ...
  return (
    <div className="relative w-full max-w-sm">
      <div className="relative group">
        <span className="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-on-surface-variant group-focus-within:text-accent transition-colors">search</span>
        <input
          type="text"
          ...
          className="w-full bg-surface-container-high border border-outline-variant/30 rounded-2xl pl-12 pr-4 py-3 text-sm font-bold text-on-surface focus:ring-2 focus:ring-accent/30 focus:border-accent outline-none transition-all placeholder:text-on-surface-variant/40"
        />
      </div>

      {isOpen && (query.length >= 2 || loading) && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-outline-variant/20 rounded-2xl shadow-2xl z-[100] overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
          {loading ? (
            <div className="p-4 text-center text-xs font-bold text-on-surface-variant animate-pulse italic">Searching customers...</div>
          ) : results.length === 0 ? (
            <div className="p-4 text-center text-xs font-bold text-on-surface-variant">No customers found</div>
          ) : (
            <div className="divide-y divide-outline-variant/5">
              {results.map(r => (
                <button
                  ...
                  className="w-full flex items-center justify-between p-4 hover:bg-accent/5 transition-colors text-left group"
                >
                  <div>
                    <p className="font-bold text-tertiary group-hover:text-accent transition-colors">{r.name}</p>
                    <p className="text-[10px] text-on-surface-variant font-mono">{r.customerId}</p>
                  </div>
                  <span className="material-symbols-outlined text-accent opacity-0 group-hover:opacity-100 transition-all -translate-x-2 group-hover:translate-x-0">chevron_right</span>
                </button>
              ))}
            </div>
          )}
...
export default function RepaymentTracker() {
  ...
  const [closureModal, setClosureModal] = useState<{ loan: Repayment; totalPenaltyDue: number } | null>(null);
  ...
  const handlePaymentSuccess = (closure?: ClosureResponse) => {
    if (closure?.required && paymentModal) {
      setClosureModal({ loan: paymentModal, totalPenaltyDue: closure.totalPenaltyDue });
    }
  ...
  return (
    <div className="pb-10">
                    ...
                    >
                      ...
                      <td className="px-4 py-4">
                        {isPenaltyPending ? (
                          <button
                            onClick={() => setClosureModal({ loan: r, totalPenaltyDue: r.penaltyDue || 0 })}
                            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-100 text-amber-700 text-[10px] font-bold hover:bg-amber-500 hover:text-white transition-all active:scale-95"
                          >
                            <span className="material-symbols-outlined text-sm">price_check</span>
                            Settle
                          </button>
      ...
      {closureModal && (
        <PenaltyClosureModal
          loan={closureModal.loan}
          totalPenaltyDue={closureModal.totalPenaltyDue}
